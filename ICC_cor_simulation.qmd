...
icc1_cor <- function(Y, unit = c("single", "average"), rep_size = 1){
  unit <- match.arg(unit)
  N <- nrow(Y)
  M <- ncol(Y)
  
  icc1 <- future_map_dbl(seq_len(rep_size), \(i) {
    ...
    if (unit == "single") {
      ...
      icc1 <- cor(Y_j1s_mean, Y_j2s_mean)
...
run_case1_sim <- function(N, M, mu, sigma_S, sigma_E) {
  ...
  cor_1_output <- icc1_cor(Y, unit = "single", rep_size = 1000)
  cor_k_output <- icc1_cor(Y, unit = "average", rep_size = 1000)
...
icc2_cor <- function(Y, 
    ...
    icc2 <- future_map_dbl(seq_len(rep_size), \(i) {
    ...
    if (type == "agreement") {
      if (unit == "single") { # ICC(A, 1)
        ...
        Y_j1s_mean <- rowMeans(Y_j1s_matrix, na.rm = TRUE)
        Y_j2s_mean <- rowMeans(Y_j2s_matrix, na.rm = TRUE)
        icc2 <- cor(Y_j1s_mean, Y_j2s_mean)
...
run_case2_sim <- function(N, M, mu, sigma_S, sigma_R, sigma_E) {
  ...
  cor_A1_output <- icc2_cor(Y, type = "agreement", unit = "single", rep_size = 1000)
  cor_Ak_output <- icc2_cor(Y, type = "agreement", unit = "average", rep_size = 1000)
  cor_C1_output <- icc2_cor(Y, type = "consistency", unit = "single", rep_size = 1000)
  cor_Ck_output <- icc2_cor(Y, type = "consistency", unit = "average", rep_size = 1000)
...
::: {.callout-warning}
There is a noticeable difference between icc-based and cor-based estimates, especially when $\sigma_R / \sigma_E$ is large.
:::
